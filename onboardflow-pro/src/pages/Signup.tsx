...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Prepare user data matching the User interface
      const userData = {
        name: formData.fullName, // fullName from form maps to name for backend
        email: formData.email,
        mobile: formData.mobile,
        password: formData.password,
      } as User;

      console.log('[SIGNUP-COMPONENT] Calling signup function...');
      const result = await signup(userData);
      console.log('[SIGNUP-COMPONENT] Signup result:', result);
      
      if (result.success) {
        toast.success("Account created successfully!");
        
        // Store token and user data in your app state/context
        // For now, you can store in localStorage temporarily
        if (result.token) {
          localStorage.setItem('auth_token', result.token);
        }
        if (result.userData) {
          localStorage.setItem('kyc_user', JSON.stringify(result.userData));
        }
        
        console.log('[SIGNUP-COMPONENT] Navigating to dashboard...');
        navigate("/kyc-dashboard");
      } else {
        console.error('[SIGNUP-COMPONENT] Signup failed:', result.error);
        toast.error(result.error || "Failed to create account");
      }
    } catch (error) {
      console.error('[SIGNUP-COMPONENT] Exception during signup:', error);
